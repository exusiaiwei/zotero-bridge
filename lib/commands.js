...
export async function highlight(key, text, { comment, color = "#2ea8e5", page } = {}) {
  const { findTextRects } = await import("./pdf.js");
  ...
  const info = await bridge.execute(`
    ...
    for (var aid of attIDs) {
      var att = Zotero.Items.get(aid);
      if (att && att.attachmentContentType === 'application/pdf') {
        var path = await att.getFilePathAsync();
        return {pdfKey: att.key, pdfId: att.id, path: path};
      }
    }
    return {error: 'no PDF attachment', hint: 'Use find-pdf first.'};
  `);

  if (!info || info === null) return { error: `Item not found: ${key}`, hint: NOT_FOUND_HINT };
  if (info.error) return info;

  const pageIndex = page != null ? page - 1 : undefined;
  const match = await findTextRects(info.path, text, { pageIndex });

  if (!match) {
    return {
      error: `Text not found in PDF: "${text.substring(0, 50)}"`,
      hint: page ? `Searched page ${page}. Try without --page to search all pages.` : "Searched all pages.",
    };
  }

  const sortIndex = String(match.pageIndex).padStart(5, "0") + "|000000|00000";
  const position = JSON.stringify({ pageIndex: match.pageIndex, rects: match.rects });
  const commentJs = comment ? `annot.annotationComment = '${esc(comment)}';` : "";

  return bridge.execute(`
    var att = Zotero.Items.get(${info.pdfId});
    var annot = new Zotero.Item('annotation');
    annot.libraryID = 1;
    annot.parentID = att.id;
    annot.annotationType = 'highlight';
    annot.annotationText = '${esc(match.matchedText)}';
    ${commentJs}
    annot.annotationColor = '${esc(color)}';
    annot.annotationPageLabel = '${match.pageIndex + 1}';
    annot.annotationSortIndex = '${sortIndex}';
    annot.annotationPosition = '${esc(position)}';
    await annot.saveTx();
    return {ok: true, key: annot.key, page: ${match.pageIndex + 1}, text: '${esc(match.matchedText)}'};
  `);
}
